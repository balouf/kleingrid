...
def python_edt(n=1000, r=2, p=1, q=1, n_runs=10000, numba=True, parallel=False):
    ...
    parallel: :class:`bool`, default=False
        Parallelize runs. Use for single, lengthy computation. Coarse-grained (high-level) parallelisation is preferred.
    ...
    Result(edt=85.93, computation_time=..., n=1000, r=0.5, p=1, q=1, n_runs=100, numba=False, parallel=False)
    >>> python_edt(n=1000, r=1, n_runs=100, numba=False)  # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
    Result(edt=69.44, computation_time=..., n=1000, r=1, p=1, q=1, n_runs=100, numba=False, parallel=False)
    >>> python_edt(n=1000, r=1.5, n_runs=100, numba=False)  # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
    Result(edt=55.8, computation_time=..., n=1000, r=1.5, p=1, q=1, n_runs=100, numba=False, parallel=False)
    >>> python_edt(n=1000, r=2, n_runs=100, numba=False)  # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
    Result(edt=67.28, computation_time=..., n=1000, r=2, p=1, q=1, n_runs=100, numba=False, parallel=False)
    >>> python_edt(n=1000, r=2.5, n_runs=100, numba=False)  # doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
    Result(edt=167.85, computation_time=..., n=1000, r=2.5, p=1, q=1, n_runs=100, numba=False, parallel=False)
    >>> python_edt(n=10000000000, r=1.5, n_runs=10000, p=2, q=2, parallel=True)  # doctest: +SKIP
    Result(edt=6823.2369, computation_time=..., n=10000000000, r=1.5, p=2, q=2, n_runs=10000, numba=True, parallel=True)
    """
    start = time.time()
    if r < 1:
        gen = draw_r_lt_1(n, r)
